  ...
  return async function hologramFetch(input: string, init?: RequestInit): Promise<Response> {
    ...
    for (let tick = 0; tick < MAX_TICKS; tick++) {
      ...
      if (parsed.complete) {
        ...
        return new Response(parsed.body && parsed.body.length ? (parsed.body as BodyInit) : null, {
...
export class HoloSocket {
  ...
  constructor(bridge: GuestBridge, port: number, url: string, schedule: (fn: () => void) => void = (fn) => setTimeout(fn, 0)) {
    this.bridge = bridge;
